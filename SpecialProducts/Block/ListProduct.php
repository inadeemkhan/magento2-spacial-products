...
class ListProduct extends \Magento\Catalog\Block\Product\AbstractProduct implements IdentityInterface
{
    ...
    protected function _getProductCollection()
    {
        ...
        if (isset($category)) {
            ...
            if ($categoryId == 1452) {
                $this->_productCollection->addAttributeToFilter('is_special_offer', 1);
            }
            if ($categoryId == 99) {
                $this->_productCollection->addAttributeToFilter('is_bestseller', 1);
            }
        }
    ...
    private function initializeProductCollection()
    {
        ...
        if (isset($category)) {
            $categoryId = $category->getId();
            $categoryArray = $this->_categoryFactory->create()->load(1483)->getAllChildren(true);
            if (in_array($categoryId, $categoryArray)) {
                $collection->addAttributeToFilter('is_special_offer', 1);
                // $collection->addAttributeToFilter('is_shown_on_homepage', 1);
            }
        }
